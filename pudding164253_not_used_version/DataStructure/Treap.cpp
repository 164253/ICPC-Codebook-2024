...
struct node{
    ...
    int mx,mn,sum;
    int rev_tag,add_tag;
    node(int x):val(x),l(0),r(0),sz(1),rev_tag(0),add_tag(0),mx(x),mn(x),sum(x){}
    node(node*tr):val(tr->val),l(tr->l),r(tr->r),sz(tr->sz),rev_tag(tr->rev_tag),add_tag(tr->add_tag),mx(tr->mx),mn(tr->mn){}
    void pull(){
        sz=1;mx=mn=sum=val;
...
node*insertkth(node*tr,int k){
    ...
    return merge(merge(l,new node(0)),r);//new node拿來區間操作初始化
}
...
void modify(node*&tr,int L,int R,int v){//modify[L,R]
    auto[x,r]=splitsz(tr,R);
    auto[l,m]=splitsz(x,L-1);
    m->val+=v;m->add_tag+=v;m->rev_tag=1;//看要改啥
    tr=merge(merge(l,m),r);
}
int main(){
    int t;node*tr=0;
    for(cin>>t;t--;){
        ...
        cin>>op>>x;
        switch(op){
            case 1:tr=insert(tr,x);break;
            case 2:tr=eraseone(tr,x);break;
            case 3:cout<<rnk(tr,x-1)+1<<"\n";break;
            case 4:cout<<kth(tr,x)<<"\n";break;
            case 5:cout<<prev(tr,x)<<"\n";break;
            case 6:cout<<next(tr,x)<<"\n";break;
        }
    }
}