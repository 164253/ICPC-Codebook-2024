...
struct Line{
    double a,b;//ax+b
    Line(double _a=-1,double _b=-1e6):a(_a),b(_b-_a){}
...
void ins(Line l,int L=1,int R=N,int id=1){
    if(L==R){if(seg[id].a<0||l(M)>seg[id](M))seg[id]=l;return;}
    if(l(M)>seg[id](M))swap(l,seg[id]);
    if(l.a>seg[id].a)ins(l,M+1,R,rid);
    else ins(l,L,M,lid);
}
...
int main(){
    ...
    for(cin>>n;n--;){
        string s;
        cin>>s;
        if(s[0]=='Q'){int x;cin>>x;cout<<max(0,((int)(qry(x)*100))/10000)<<"\n";}
        else{double s,p;cin>>s>>p;ins(Line(p,s));}
    }
}